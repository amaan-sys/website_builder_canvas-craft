      ...
      <section className="relative" style={{ background, padding }}>
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => (<div key={stat.id || index} className="text-center"><div className="text-6xl md:text-7xl font-bold text-white mb-2">{stat.value}<span className="text-blue-400">{stat.suffix || ''}</span></div><div className="text-slate-300 text-sm uppercase tracking-wider">{stat.label}</div></div>))}
          </div>
        </div>
      </section>
    );
